...
import { CouncilSession, Message, AgentPerspective, UserSettings, SeniorityLevel, CouncilTone, SpecialRoles } from './types';
          ...
          <div className="absolute inset-0 z-50 bg-[#232f3e]/60 backdrop-blur-sm flex items-center justify-center p-4">
             <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl p-8">
                <div className="flex justify-between items-center mb-6">
                  <h2 className="text-xl font-bold">Council Settings</h2>
                  <X className="cursor-pointer" onClick={() => setIsSettingsOpen(false)} />
                </div>
                <div className="space-y-4">
                  <p className="text-sm text-slate-500">Selected LPs determine your Council members.</p>
                  <div className="grid grid-cols-2 gap-2">
                    {AMAZON_LPS.map(lp => (
                      ...
                      }} className={`text-xs p-2 rounded border ${settings.lpFocus.includes(lp) ? 'bg-orange-100 border-orange-500 text-slate-900' : 'bg-white text-slate-600'}`}>{lp}</button>
                    ))}
                  </div>
                </div>
                <button onClick={() => setIsSettingsOpen(false)} className="mt-8 w-full bg-[#232f3e] text-white py-3 rounded-xl font-bold">Confirm</button>
             </div>
          </div>
        )}

        {activeSession && (
          <div className="bg-white border-b px-8 py-4 flex flex-col md:flex-row items-center justify-between gap-4">
            ...
            <div className="flex bg-slate-100 p-1 rounded-xl">
              <button onClick={() => setActiveTab('chat')} className={`px-6 py-2 rounded-lg text-sm font-semibold ${activeTab === 'chat' ? 'bg-white shadow text-[#ff9900]' : 'text-slate-500'}`}>Discovery</button>
              <button onClick={() => setActiveTab('council')} className={`px-6 py-2 rounded-lg text-sm font-semibold ${activeTab === 'council' ? 'bg-white shadow text-[#ff9900]' : 'text-slate-500'}`}>Review</button>
              <button onClick={() => setActiveTab('documents')} className={`px-6 py-2 rounded-lg text-sm font-semibold ${activeTab === 'documents' ? 'bg-white shadow text-[#ff9900]' : 'text-slate-500'}`}>Docs</button>
            </div>
          </div>
        )}
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                  {activeSession.messages.map(m => (
                    <div key={m.id} className={`flex ${m.sender === 'User' ? 'justify-end' : 'justify-start'}`}>
                      <div className={`max-w-[85%] rounded-2xl px-6 py-4 border ${m.sender === 'User' ? 'bg-[#232f3e] text-white' : 'bg-white border-slate-200 text-slate-800'}`}>
                        <span className="text-[10px] font-bold uppercase tracking-widest text-[#ff9900] block mb-1">{m.sender === 'User' ? 'PM' : 'MASTER AGENT'}</span>
                        <div className="text-sm chat-markdown"><ReactMarkdown>{m.content}</ReactMarkdown></div>
                      </div>
                    </div>
                  ))}
                  {isChatLoading && <div className="animate-pulse text-slate-400 text-xs font-bold uppercase">Master Agent is thinking...</div>}
                  <div ref={chatEndRef} />
                </div>
                <form onSubmit={handleSendMessage} className="p-6 border-t bg-slate-50 flex gap-4">
                  <input 
                    type="text" 
                    value={inputValue} 
                    onChange={e => setInputValue(e.target.value)} 
                    placeholder="Type to refine facets (Customer, Problem, Solution...)" 
                    className="flex-1 bg-white border rounded-xl px-6 py-4 text-sm text-slate-900 focus:outline-none focus:ring-2 focus:ring-[#ff9900]/40 placeholder:text-slate-400" 
                  />
